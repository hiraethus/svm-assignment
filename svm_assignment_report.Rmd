...
```{r}
svm.model <- svm(group ~ ., data = training.set)
```
...
```{r}
tuned.svm <- tune.svm(group ~ ., data = training.set, cross = 50)
...
```{r}
best.svm <- svm(group ~ ., data = training.set, cosr = cost, gamma = gamma)
best.pred <- predict(best.svm, test.set[,-1])
best.prediction.tab <- table(pred = best.pred, true = test.set[,1])
best.prediction.tab
```

We can also compare the performance of our k-fold cross validated svm by plotting it against our original SVM on a ROC graph.
